...
//    }
    ...
    @PostMapping("pending")
    public ResponseJSONResult pending(
    ...
    public ResponseJSONResult saveList(
        ...
        if(orderItemsCommentBO.isEmpty() || orderItemsCommentBO.size() == 0 || orderItemsCommentBO == null){
            return ResponseJSONResult.errorMsg("评价内容不能为空");
        }
    ...
    @PostMapping("query")
    public ResponseJSONResult commentLevel(@ApiParam(name = "userId",value = "用户id",required = true)
                                        @RequestParam String userId,
